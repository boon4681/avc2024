...
function flood(cx, cy, plantType) {
    ...
    for (let i = 0; i < directions.length; i++) {
        ...
        if (validb(nx, ny) && data[nx][ny] == plantType) {
            ...
            const [a, b] = [directions[(i + 1) % 4], directions[(i + 3) % 4]]
            const k = [a[0] + cx, a[1] + cy, i].join(",")
            const y = [b[0] + cx, b[1] + cy, i].join(",")
            const j = [cx, cy, i].join(",")
            if (!perims[k] && !perims[y]) {
                perimeter++;
            } else if (perims[k] && perims[y]) {
                perimeter--
            }
            perims[j] = true
        }
